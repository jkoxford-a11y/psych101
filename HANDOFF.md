...
5. **Labs — five built and entered in `docs/labs/index.html` as of Session 70 (2026-07-04); pattern well-established:**
   ...
   - `docs/labs/ch07/classical-conditioning.html` — complete, linked from ch07; index card fixed this session (was still pointing at the pre-move `ch06/` path)
   - `docs/labs/ch09/fluid-intelligence-rule-finding.html` — new this session (Session 70), wired into `docs/labs/index.html` and linked from both `source/chapters/ch09-...md` and `docs/chapters/09-...html`; verified functionally (see Current Status)
   - `docs/labs/lab-design-spec.md` — pattern spec (Session 40)
   - **Still open:** `docs/labs/index.html`'s own sidebar nav uses stale pre-Session-59 chapter numbers/hrefs (flagged Session 70, not fixed); the empty `docs/labs/ch06/` folder still needs manual deletion (OneDrive permission block, several sessions running).
   - Next lab: Jon to decide (Ch2/Ch7/Ch9 pattern is proven; no chapter currently has a work-order queued for a new lab).
