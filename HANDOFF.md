...
2c. **Spine-alignment backlog for Ch10–Ch13 — Ch8 Memory (Session 66) and Ch9 Thinking/Language/Intelligence (Session 68, now v0.3 after a full Language/Intelligence throughline rewrite) both done, Ch10 next.** `pipeline/theoretical-spine.md` is correctly reconciled to the locked chapter numbers (see Current Status), so its §7 drafting rules and §8 Layer-3 notes can be used directly to audit any remaining chapter without Jon drafting a bespoke work order — validated on Ch8 (now v0.3) and Ch9 (now v0.2, both awaiting review). Same treatment still needed for: Ch10 Lifespan Development (next up), Ch11 Social Psychology (+ the still-pending Personality content fold-in, see item 3 below), Ch12 Emotion/Stress/Coping, Ch13 Disorders & Therapy. Pattern per chapter: grep for existing spine language first, read the chapter fully, check against spine §7 + that chapter's §8 notes, propose specific edits before applying, verify any new citation live before adding it. **Note for the Ch10 pass:** Ch9 (Session 68) added a short assimilation/accommodation bridge to Ch9 Section 1 that explicitly cross-references Ch10's existing Piaget coverage rather than duplicating it — worth a quick look when auditing Ch10 to confirm the cross-reference still makes sense after that chapter's own spine pass. **Also flagged this session:** Ch9 was already well over the 3,500–5,000 word target before this pass (~8,000+ words) and is now longer still — Jon wants to see the full chapter before deciding what to cut or move into a lab; this is an open decision, not yet resolved.
