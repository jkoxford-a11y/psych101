...
2c. **Spine-alignment backlog for Ch10–Ch13 — Ch8 Memory (Session 66), Ch9 Thinking/Language/Intelligence (Session 68), Ch10 Lifespan Development (Session 69, now v0.4), and Ch12 Emotion/Stress/Coping (Session 72, now v0.3) all done. Ch11's theory groundwork is done (Session 71) but the chapter prose edit itself has not started — that plus Ch13 Disorders & Therapy are what's left.** `theoretical-spine.md` §8 and `theoretical-foundations.md` §22 have everything needed to draft Ch11's spine-alignment pass and the Personality merge into it — no further literature research required before starting. `pipeline/theoretical-spine.md` is correctly reconciled to the locked chapter numbers (see Current Status), so its §7 drafting rules and §8 Layer-3 notes can be used directly to audit any remaining chapter without Jon drafting a bespoke work order — validated on Ch8, Ch9, Ch10, and now Ch12 (all awaiting Jon's review). Same treatment still needed for: Ch11 Social Psychology (+ the still-pending Personality content fold-in, see item 3 below — next up chronologically since its groundwork is furthest along) and Ch13 Disorders & Therapy. Pattern per chapter: grep for existing spine language first, read the chapter fully, check against spine §7 + that chapter's §8 notes, propose specific edits before applying, verify any new citation live before adding it. **Ch10's pass (Session 69) was bigger than Ch8/Ch9's** — the spine's Layer-3 notes for Ch10 (life history theory, secondary altriciality, cooperative breeding, Neanderthal comparison, teaching-as-adaptation, biobehavioral synchrony) were entirely absent from the chapter, not just lightly connected, so this wasn't a quick connective-tissue pass. New precedent set there worth reusing: when a chapter's assigned spine question doesn't match its actual opening framing, fold the spine material into whichever existing tension/theme it gives real content to, rather than adding it as a competing section — Jon's call, made explicitly for Ch10, but likely a good default for Ch11/Ch13 too if the same mismatch shows up. **Ch12's pass (Session 72) fit a different pattern than Ch9/Ch10** — the chapter needed almost no framework-level correction (already fully Barrett-constructionist) but had two genuine, isolated content gaps (cognitive light cone, hedonic treadmill) that the spine's own Ch12 notes call out specifically; also surfaced and fixed a citation-attribution error inside the spine's own Layer-3 notes (the SM case study's behavioral details were attributed to the wrong paper — see Session 72 log) and a structural gap (no Connections table) unrelated to spine content but worth fixing in the same pass. **Also flagged, still open, not yet resolved for any of these three chapters:** Ch9 was already well over the 3,500–5,000 word target before its pass (~8,000+ words) and is now longer still; Ch10 grew by ~500–600 words on an already-substantial base; Ch12 was already ~8,000 words before its pass and is now higher too. Jon wants to see full chapters before deciding what to cut or move into labs — open decision for a future session.
