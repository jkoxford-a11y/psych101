...
4. **Decide the Ch. 13 HTML builder question** (below). Ch. 13's HTML remains a week stale and cannot be regenerated by the shared builder.
...
## Blocker carried forward — the shared builder does not know about Chapter 13

`pipeline/build_chapter_html.mjs` has configs for chapters **2, 3, 4, 5, 8, 9, 10, 11, 12 only**. `node pipeline/build_chapter_html.mjs 13` fails with `Unknown chapter: 13`. Chapters **1, 6, 7, 13 and the prologue** have HTML in `docs/chapters/` the shared builder cannot regenerate.

- `docs/chapters/13-disorders-therapy.html` was last written **2026-07-19** and does not contain the settled Ch. 13 spine work.
- **`lint_chapters.py` reads `docs/chapters/*.html`, not the Markdown source.** Its `13-disorders-therapy.html PASS` is a pass on a stale artifact and says nothing about the current source. Same caveat for Ch. 1, 6, 7 and the prologue.

The fix is a builder config block for Ch. 13, but the config carries per-chapter flags (`matchAnyItalicFigureCaption`, `convertStructuredH4Callouts`, `minimumSectionNavLinks`) that are real formatting decisions, not boilerplate. Needs a pass that reads the existing Ch. 13 HTML and sets them deliberately — and the same pass could close Ch. 1, 6, 7 and the prologue at once.
