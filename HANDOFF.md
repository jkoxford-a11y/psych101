...
2c. **Spine-alignment backlog for Ch9–Ch13 — Ch8 Memory piloted and done (Session 66), Ch9 next.** `pipeline/theoretical-spine.md` is now correctly reconciled to the locked chapter numbers (see Current Status), so its §7 drafting rules and §8 Layer-3 notes can be used directly to audit any remaining chapter without Jon drafting a bespoke work order — validated this session on Ch8 Memory (now v0.3, awaiting review). Same treatment still needed for: Ch9 Thinking/Language/Intelligence (next up), Ch10 Lifespan Development, Ch11 Social Psychology (+ the still-pending Personality content fold-in, see item 3 below), Ch12 Emotion/Stress/Coping, Ch13 Disorders & Therapy. Pattern per chapter: grep for existing spine language first (all six had zero as of Session 66), read the chapter fully, check against spine §7 + that chapter's §8 notes, propose specific edits before applying, verify any new citation live before adding it.
