...
2b. **~~Ch. 7 (Learning) spine re-alignment~~ — DONE (Session 65).** `ch07-learning.md` is now v0.4: Section A confirmed already applied (Session 61 sweep), Sections B/C applied fresh this session (spine question fronts Where This Fits, master-loop naming, extinction trade-off labels, compression-framing sentence, wanting/liking distinction with Berridge & Robinson 1998, spine unifying line in Chapter Summary). Sections D–F logged to `source/visuals-inventory.md` and `docs/teaching/demo-ideas.md`. **New follow-up items surfaced, not yet done:** (a) delete `ch07-learning_SPINE-REALIGNMENT_TEMP.md` manually (bash can't reach it, OneDrive restriction); (b) decide whether to rename `docs/labs/ch06/` → `ch07/` (it holds the Learning conditioning lab but now collides with Sleep's chapter number) and add a link to it from `ch07-learning.md` (currently unlinked, a real content gap not a placeholder); (c) optionally remove the orphaned Bandura (1963) References entry, no longer cited in-text since v0.2.
