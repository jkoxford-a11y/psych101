...
2. Read the chapter's pre-review audit in `pipeline/audits/`. Its **must-preserve and must-correct findings are content commitments and stand** — if a seed would touch a protected asset, say so rather than quietly routing around it. Its **register and hedging prescriptions do not stand**; `AGENTS.md` → `## Style` governs those, and an audit conflicting with it is stale by definition. See `AGENTS.md` → *Audits are diagnostic, not authoritative*.
