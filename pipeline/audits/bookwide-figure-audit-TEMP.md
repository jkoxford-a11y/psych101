...
| F-016 | P2 | Figure 4.4 | RESOLVED 2026-08-12 | “Eye” versus “brain” creates a false boundary for opponent processing, which begins in downstream retinal circuitry and continues centrally. | Regenerated as v2 (GPT image generation, instructor-reviewed through two rounds): replaced the “Eye”/“Brain” location labels with an explicit Retina → LGN → Cortex pathway, and corrected the cone-response curves to realistic overlapping S/M/L peaks, removing a duplicated-curve rendering artifact in the process. Caption and alt text updated to match. Passed the five-viewport expansion check (no page or image overflow at 1366×768, 1024×768, 812×375, and 375×812, in addition to native desktop). | S–M |
| F-017 | P2 | Figure 4.6 | RESOLVED 2026-08-16 | The former artwork asserted “same pixels” while showing different rendered side scenes, so the demonstration itself did not preserve identical input. | Replaced the artwork with one fixed digital image shown once, branching into two alternative illumination assumptions and two inferred surface-color interpretations. The active chapter asset path was replaced in place, so chapter Markdown and generated HTML required no path change; the corrected 1600×900 figure was visually inspected before installation. | M |
| F-018 | P2 | Figure 7.2 | RESOLVED 2026-08-16 | The former artwork made the “old prediction” visibly weaken during extinction even though the figure's central claim was that the association remains available. | Replaced it with a three-stage infographic that keeps the earlier Bell → Food relationship visible, adds context-sensitive Bell + extinction context → No food learning, and labels the single lower curve as observed responding rather than association strength. A clock-to-new-context cue distinguishes return after time or context change. Caption, alt text, figure metadata, and durable inventory were synchronized. The rebuilt chapter passed the linter; the coherence checker had no hard failures; expanded/collapsed browser checks at 1366×768, 1024×768, 812×375, 390×844, and 375×812 found no horizontal overflow or console errors. | M |
...
| 4 | Figures 4.2, 4.4, 4.5, 4.6, and 4.9 resolved. Remaining: the 4.8 numbering decision. |
