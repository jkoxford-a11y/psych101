...
- Severity: BLOCKER
- Issue summary: This is an editable scaffold, not a finished figure. The canvas is almost blank except for a title and the note "Editable SVG scaffold; use the PNG for the polished raster version."
- Recommended fix: Replace with a finished SVG or remove/retire the scaffold from use.
...
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Dark background with low-contrast dark heading and dense bottom explanatory text. The visual is usable but conflicts with the light, sparse project style.
- Recommended fix: Redraw on a white/light background, increase contrast, and move the long bottom explanation into the caption.
- Fix type: Mechanical redraw from existing content.

### `docs/images/ch02/fig2-6-random-sampling-vs-assignment.svg`
- Render status: rendered
- Severity: BLOCKER
- Issue summary: Editable scaffold only; mostly blank with a title and scaffold note.
- Recommended fix: Replace with a complete SVG or retire.
...
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Portrait-ish aspect ratio inside a 16:9 preview, and some scatter points appear near or beyond the panel edges. It also uses a different older style than current project figures.
- Recommended fix: Redraw as a 16:9 four-panel dartboard figure with consistent margins and point clouds contained within targets.
- Fix type: Mechanical.

### `docs/images/ch03/fig_stress_response_fast_slow.svg`
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Old monochrome/monospaced style, excessive top whitespace, and a long key-idea sentence inside the figure. Labels are readable but not visually aligned with current style.
- Recommended fix: Convert to the current light-card style, tighten layout, and move the key idea to the caption.
- Fix type: Mechanical redraw from current content.

### `docs/images/ch03/3 images not used/fig_action_potential_threshold.svg`
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Old monospaced labels, dense annotation, and a long bottom key-idea sentence inside the figure. The science is legible, but the figure is not sparse.
- Recommended fix: Simplify labels to threshold/spike/refractory period, enlarge labels, and move nuance to caption.
- Fix type: Mechanical.

### `docs/images/ch03/3 images not used/fig_brain_imaging_tradeoffs.svg`
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Table-heavy layout with many small cells. It violates the "figure should not contain paragraphs" standard and may be better as a table than a figure.
...
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Old monochrome style and several very small labels (for example repeated node labels). Dense enough that it would be weak for slide/textbook use.
- Recommended fix: Redraw with fewer labels and larger text.
- Fix type: Mechanical.

### `docs/images/ch03/3 images not used/fig_neurotransmitter_vs_hormone.svg`
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Older monochrome style, small labels, and too much explanatory text inside boxes.
...
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Dense labels/callouts. The core logic is useful, but the bottom explanatory box is too wordy for the figure itself.
- Recommended fix: Simplify to input → hemisphere → response logic, and move caveat text to caption.
- Fix type: Mechanical.
...
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Very dense labels, explanatory paragraph box, crowded frequency labels, and a bottom apex label close to the canvas edge. It tries to teach place theory and frequency theory at once.
- Recommended fix: Split into two simpler figures or redraw with much less text.
...
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Multiple long explanatory labels, dense footer text, and heavy arrow traffic through the central gate. Usable as a lecture diagram, but too text-heavy for the style guide.
- Recommended fix: Reduce labels to stimulus/touch/gate/brain/descending modulation, and move examples/caveats to caption.
- Fix type: Mechanical.

### `docs/images/ch04/unused/fig_gestalt_principles.svg`
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Five mini-figures with small labels. It is crowded and table-like, though conceptually coherent.
...
- Render status: rendered
- Severity: FIX SOON
- Issue summary: Crowded left eye diagram; orange fovea label overlaps the eye and competes with the chart. The right chart is readable but visually busy.
- Recommended fix: Simplify eye-side labels, move fovea/blind-spot details into cleaner callouts, and increase spacing between eye and chart.
- Fix type: Mechanical.
...
- Render status: rendered
- Severity: ACCEPTABLE
- Issue summary: Fixed in layout pass. RHT/SCN labels were moved clear of the brain silhouette, pathway strokes were slimmed, and pineal/melatonin callouts were spaced away from the arrowhead and molecule cluster.
- Recommended fix: No immediate layout fix needed. Optional future pass could move the numbered list into surrounding prose if an even sparser figure is desired.
- Fix type: Fixed mechanically; instructor review can confirm biological pathway wording.
...
- Render status: rendered
- Severity: ACCEPTABLE
- Issue summary: Fixed in layout pass. The lower "Modern update" card was enlarged, the caveat was wrapped into shorter lines, the decorative dots/squiggle were removed, and the concrete operational title was tightened to avoid edge clipping.
- Recommended fix: No immediate layout fix needed. Stage-card content remains intact, with the single top progression arrow preserved.
- Fix type: Fixed mechanically.
...
## Priority Layout Fixes

- `docs/images/ch05/fig_scn_light_to_melatonin_bioinfographic.svg` - fixed RHT/SCN label placement, slimmed pathway arrows, and moved pineal/melatonin labels away from the arrowhead and molecule cluster. Rendered to 1600 x 900 PNG and verified no clipped text, label overlap, arrowhead collision, or low-contrast text.
- `docs/images/ch10/ch10_piaget_stages_sequence_with_caveat.svg` - fixed the lower Modern update card overflow by enlarging the card, removing the decorative dots/squiggle, wrapping the caveat into shorter lines, and tightening the concrete operational title. Rendered to 1600 x 900 PNG and verified no clipped text, label overlap, arrowhead collision, or low-contrast text.
...
- BLOCKER files: 4
- FIX SOON files: 13
- ACCEPTABLE/minor-issue files: 32

Top 5 files needing attention:

1. `docs/images/ch02/fig2-5-evidence-ladder.svg` — scaffold-only, not a finished figure.
2. `docs/images/ch02/fig2-6-random-sampling-vs-assignment.svg` — scaffold-only, not a finished figure.
3. `docs/images/ch02/fig2-7-operational-definition-reliability-validity.svg` — scaffold-only, not a finished figure.
4. `docs/images/ch02/fig2-8-type-i-type-ii-error-matrix-ai.svg` — scaffold-only, not a finished figure.
5. `docs/images/ch02/fig2-5-sampling-vs-assignment.svg` — low-contrast/dense older style.
