...
3. **Explain** how episodic memories become semantic knowledge, and use H.M.'s case to distinguish explicit from implicit memory.
...
- **Long-term memory (LTM)** — a very large-capacity store where some memories persist for decades; where information ends up if it survives the earlier stages
...
![Figure 8.1 — A three-panel flow diagram titled Classic Atkinson-Shiffrin Model shows information moving from Sensory Memory, with high capacity and a duration under one second, to Short-Term/Working Memory, labeled sharply limited with the classic estimate of seven plus or minus two items and a duration of seconds to about one minute, to Long-Term Memory, with very large capacity and some memories lasting decades. Arrows labeled Attention and Encoding connect the panels moving forward; a thinner return arrow labeled Retrieval moves from Long-Term Memory back to Short-Term/Working Memory. Small downward arrows beneath each panel indicate information can be lost or not retrieved at every stage.](../images/ch08/ch08_atkinson_shiffrin_three_box_model.svg)
*Figure 8.1. The classic Atkinson-Shiffrin model maps memory as three stages: a brief, high-capacity sensory buffer; a sharply limited short-term store; and a very large long-term store, where some memories persist for decades. The boxes represent functionally distinct stages, not physical containers.*
...
Section 3 ended with compression as a feature. Here is what it costs. A system that keeps the gist and lets the particulars go has to rebuild the particulars on demand — and rebuilding is guessing, however good the guess. That guessing requires no brain damage at all, and it happens to everyone. Memory is reconstructive, and the clearest evidence for that claim comes from cases where reconstruction visibly goes wrong, producing confident, detailed, sincerely-held memories that are nonetheless false, distorted, or borrowed from the wrong source entirely.
...
**Try it yourself:** In [Did You Really See That?](../../docs/labs/ch08/misinformation-effect.html), read a brief campus incident, answer follow-up questions that embed false details in their phrasing, and take a memory test to see whether post-event information shifted what you recalled from the original scene. Treat your individual result as a demonstration to reason about, not a diagnostic measure.
