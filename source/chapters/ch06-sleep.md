...
> Draft v0.1 — 2026-07-02 | Status: Split from old `ch05-states-of-consciousness.md` v0.4 (Session 59). This chapter now inherits only the sleep material: circadian rhythms, sleep architecture and function, dreams, sleep disorders, and sleep deprivation. Content that moved to the new Ch. 5 (Consciousness) has been removed here: the attention/inattentional-blindness bridge, the full psychoactive-drug taxonomy (Section 3 of the old file), the predictive-model derivation, the standalone Seth "controlled hallucination" treatment, and the full Charles Bonnet syndrome paragraph. New sleep-specific misconception opener added; Ch. 5 bridge added in the opener and "Where This Fits." Chapter cross-references updated to the 14-chapter numbering (Learning = Ch. 7, Memory = Ch. 8, Thinking/Language/Intelligence = Ch. 9, Emotion/Stress = Ch. 13, Disorders/Therapy = Ch. 14).
...
> Figures: three in `docs/images/ch06/` (copied from ch05 in Session 59) — `fig_scn_pathway.png` (Fig 6.1), `fig_two_process_sleep_model.png` (Fig 6.2), `fig_sleep_hypnogram.png` (Fig 6.3).
...
![Figure 6.2: The two-process model of sleep regulation. Process S (sleep pressure) rises across wakefulness as adenosine accumulates and falls during sleep as it is cleared. Process C (the circadian alerting signal) oscillates across the ~24-hour day, independent of how long the person has been awake. Sleep occurs when S is high and C's alerting signal is low; mismatches between the two produce the subjective experience of being awake at the "wrong" time.](../../images/ch06/fig_two_process_sleep_model.png)
*Figure 6.2. The two-process model of sleep regulation (Borbély, 1982). Original figure.*
...
The consequences of sleep loss extend beyond emotional regulation. Sleep deprivation specifically disrupts two hormones that govern appetite: **leptin**, produced by fat tissue and signaling satiety, drops; **ghrelin**, produced by the stomach and signaling hunger, rises — producing an increase in appetite and a preference for high-calorie foods that occurs independently of actual energy expenditure (Spiegel et al., 2004). Sleep-deprived individuals are also rated as measurably less attractive and less healthy-looking by naive observers viewing photographs, an effect documented in a controlled experimental study (Axelsson et al., 2010). Immune function is similarly degraded: in a controlled study, participants who slept less were significantly more likely to develop a clinical cold after being experimentally exposed to rhinovirus than those who slept more (Cohen et al., 2009). Each effect tracks the same logic: a system that is serviced during sleep, deprived of that servicing, degrades in specific and predictable ways.
...
Sleep is not the brain powering down but the brain running a specific set of jobs it can only do offline. The suprachiasmatic nucleus generates a roughly 24-hour circadian rhythm, entrained by light through the retinohypothalamic tract and expressed partly through melatonin release, and works alongside a separate homeostatic sleep drive — the two-process model — to determine when you actually feel sleepy. Because the circadian signal (Process C) and the sleep-pressure signal (Process S) are distinct, they respond to different interventions: caffeine masks Process S by blocking adenosine receptors, while light and melatonin shift Process C.

Sleep cycles through distinct stages roughly every 90 minutes — three stages of NREM sleep culminating in deep, slow-wave NREM 3, and REM sleep, marked by rapid eye movement, near-total muscle paralysis, and the most vivid dreaming — a structure first revealed by Aserinsky and Kleitman's 1953 discovery. Sleep likely serves multiple complementary functions at once: physical restoration, evolutionarily adaptive energy conservation and predator avoidance, and active memory consolidation, the last of which depends on coordinated medial temporal lobe activity — the same circuitry whose damage, in the case of H.M., produced one of the most famous findings in the history of memory research. The glymphatic system, identified in 2013, provides the clearest mechanistic account of restoration: cerebrospinal fluid physically flushes metabolic waste — including adenosine — from brain tissue in a process that cannot run at the same rate during wakefulness. Dreams, concentrated in REM sleep, may reflect the cortex's attempt to synthesize meaning from internally generated activity (activation-synthesis theory) or evolved rehearsal for threat detection (threat-simulation theory) — accounts that are not mutually exclusive, and both instances of the constructive, predictive cortex Chapter 5 described. When these systems break down, the result is a sleep disorder: insomnia, sleep apnea, narcolepsy, or a parasomnia. NREM arousal parasomnias (sleepwalking, night terrors) emerge from deep NREM sleep; REM sleep behavior disorder, a distinct parasomnia, involves failure of REM atonia and actual enactment of dream content — the disorder that matches the common misconception about sleepwalking.
