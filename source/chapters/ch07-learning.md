...
![Three-panel classical-conditioning diagram. Before conditioning, a bell is neutral while food elicits salivation. During conditioning, the bell and food are paired. After conditioning, the bell alone elicits salivation, with the NS, UCS, UCR, CS, and CR labeled.](../images/ch07/fig_classical_conditioning_ucs_cs.png)

*Figure 7.1. Classical conditioning across three phases. A cue becomes informative when it predicts a biologically significant event. The sequence is simplified: predictive contingency, not repetition alone, determines how much is learned.*
...
![Three-panel diagram of acquisition, extinction, and return of responding. Acquisition strengthens a bell-to-food prediction. Extinction adds a context-sensitive bell-to-no-food prediction that competes with the original. Spontaneous recovery or renewal shows the original relation regaining influence after time, stress, or a context change.](../images/ch07/fig_extinction_not_erasure.png)

*Figure 7.2. Extinction is new learning, not simple erasure. A new prediction competes with the original association, so responding can remain low in the extinction context yet return after time or a context change. The diagram is a teaching model, not a literal map of stored associations.*
...
![Two-by-two matrix classifying consequences by whether behavior increases or decreases and whether something is added or removed. The quadrants show positive reinforcement with a treat after sitting, negative reinforcement with a seatbelt stopping a chime, positive punishment with a water spray after jumping, and negative punishment with a phone removed after curfew.](../images/ch07/fig_operant_conditioning_2x2.png)

*Figure 7.3. The four operant categories require two questions: did the behavior increase or decrease, and was a consequence added or removed? Positive and negative mean adding and removing; reinforcement and punishment are defined by measured changes in behavior.*
...
![Four-column infographic comparing fixed-ratio, variable-ratio, fixed-interval, and variable-interval schedules with rule diagrams and cumulative-response records. Fixed ratio shows a pause followed by a rapid run, variable ratio shows high steady responding, fixed interval shows a scalloped pattern, and variable interval shows steady moderate responding. A lower graph shows an illustrative extinction decline that is more gradual after intermittent than continuous reinforcement.](../images/ch07/fig_reinforcement_schedules.png)

*Figure 7.4. Reinforcement schedules shape characteristic response patterns, not universal rate rankings. The lower comparison normalizes each condition to its own pre-extinction baseline and illustrates greater persistence after an intermittent history. Acquisition speed is not depicted; patterns depend on schedule values, task, organism, and history.*
...
![Matrix crossing two cues, taste and audiovisual signals, with two outcomes, illness and shock. Strong learning appears for taste paired with illness and audiovisual cues paired with shock, while the crossed pairings show weaker learning. A lower panel separates the observed evidence from an evolutionary interpretation.](../images/ch07/fig_garcia_koelling_preparedness.png)

*Figure 7.5. Garcia and Koelling demonstrated selective associability: rats learned taste–illness and audiovisual-cue–shock relations more readily than the crossed relations. An evolutionary history of recurrent ecological pairings is a leading explanation, not something the experiment directly measured.*
...
![Five-stage flow diagram of Bandura's 1965 design. Children observe the same aggressive model, who is then rewarded, receives no consequence, or is punished. Initial imitation is lower after model punishment. When every child is later offered the same direct incentive, imitation rises and group differences narrow.](../images/ch07/fig_bobo_doll_observational_learning.png)

*Figure 7.6. Observed consequences changed initial performance more clearly than acquisition. The later direct-incentive test revealed learned actions that were not initially performed; rewarded and no-consequence groups were broadly similar, not a simple three-step ranking.*
...
![Line graph plotting maze errors across 17 days for rats reinforced every day, never reinforced, or first reinforced on day 11. The delayed-reward group stays near the never-reinforced group through day 10, then its errors drop sharply after reinforcement begins and approach the always-reinforced group.](../images/ch07/fig_tolman_cognitive_map.png)

*Figure 7.7. Tolman and Honzik's delayed-reward result supports latent learning: experience changed what the rats knew before reinforcement made it visible in performance. The result does not establish the exact format of the internal representation.*
...
![Four-panel sequence showing an unexpected reward, cue–reward learning, a fully expected reward, and an omitted expected reward. Corresponding activity traces show a positive phasic response to unexpected reward, a shift of that response to the predictive cue, little response when the expected reward arrives, and a dip when the expected reward is omitted.](../images/ch07/fig_dopamine_prediction_error_schultz.png)

*Figure 7.9. Reward prediction error tracks whether an outcome is better than, matches, or is worse than expected. The pattern comes from a well-studied subset of dopamine neurons in specific tasks; it does not describe every dopamine signal or all learning.*
