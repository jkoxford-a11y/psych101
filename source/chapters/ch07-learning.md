...
7. Explain what dopamine's reward-prediction-error and reward-uncertainty signals actually show about anticipation and motivation, correcting the popular "pleasure chemical" myth without swapping in an equally oversimplified replacement, and use that distinction to evaluate a real-world claim about reward, motivation, or addiction.
...
Before moving into operant conditioning, it is worth fixing the line between the two clearly, because the rest of the chapter depends on keeping them separate. Classical conditioning is about associating two *stimuli* — a CS comes to predict a UCS — and the resulting response (salivating, flinching, fearing) is **elicited** — triggered by a preceding stimulus rather than chosen: Pavlov's dog does not choose to drool. Operant conditioning, covered next, is about associating a **behavior** with its consequence, and the behavior is **emitted** rather than elicited: the organism produces it, rather than having a preceding stimulus trigger it (a well-practiced operant behavior can come to feel just as automatic as a reflex, so conscious choice is not what actually separates the two). *Diagnostic question:* a child who once cried at the dentist's office now feels anxious just walking past the dentist's building, even on a day with no appointment — is this classical or operant conditioning? (Classical — the building, a stimulus, has become associated with the dental visit, another stimulus, and the resulting anxiety is an elicited conditioned response to that pairing, not a behavior being shaped by what follows it.) A child who learned that whining at the grocery store sometimes gets a candy bar, and now whines more often at the grocery store — is this classical or operant conditioning? (Operant — whining is a behavior, and its frequency is being shaped by its consequences, which is the subject of the next section.)
...
Psychologists call this selective pattern **preparedness** — a name for the pattern itself, not a claim that the experiment reconstructed evolutionary history directly. The best-supported interpretation of why the pattern exists is that natural selection has tuned different species to associate particular kinds of cues with particular kinds of consequences, because those are the pairings that mattered for survival across each species' evolutionary history. An animal that gets sick is far more likely to have eaten something toxic than to have been harmed by a sound or a flash of light, so a nervous system biased toward blaming the flavor of recent food for illness — even across a substantial delay — would learn to avoid poisons efficiently. An animal that gets physically hurt, by contrast, is far more likely to have encountered an external threat signaled by sight or sound, so pain getting associated with audiovisual cues instead fits the same logic. This is **taste aversion learning** in its most general form. The selectivity itself — taste pairing readily with illness, audiovisual cues pairing readily with shock, and not the reverse — is directly demonstrated; the evolutionary story explaining why fits the evidence well but is an inference, consistent with the evolutionary lens this book has used since Chapter 1: learning is not a single, uniform process bolted onto an otherwise blank-slate nervous system, and it is shaped differently in different domains, plausibly because different domains posed different survival problems.
...
This is **observational learning**: acquiring a new behavior by watching someone else perform it, with no direct reinforcement of the observer required at all. Bandura's own theoretical framework, social learning theory, breaks the process into four components: attention (the observer has to actually be watching), retention (the observed behavior has to be remembered), the capacity to reproduce the behavior physically, and motivation to actually perform it. That last component turned out to matter a great deal: a follow-up study found that children who watched the aggressive model get punished for the aggression were substantially less likely to imitate it themselves (Bandura, 1965) — even though, by Bandura's own account, they had still learned the behavior through observation; they were simply less motivated to perform it. The clearest evidence for that distinction came from the same study's next step: when the experimenter then simply offered the children a reward for reproducing as much of the model's behavior as they could, the previously low-imitating group caught up almost completely to the other groups — showing they had learned the behavior all along and had only been withholding its performance. Bandura called the original effect **vicarious reinforcement** (or vicarious punishment): learning that is shaped by watching someone *else* be reinforced or punished, without ever experiencing the consequence directly yourself.
...
#### Do Not Confuse: Macaque Single-Neuron Evidence vs. Human Mirror-System Evidence

Mirror neurons were identified through direct single-neuron recordings in macaque monkeys: neurons that fire both when the monkey performs an action and when it merely watches another individual perform that same action (di Pellegrino, Fadiga, Fogassi, Gallese, & Rizzolatti, 1992). The "human mirror system" is a different, weaker kind of evidence — brain-imaging studies showing that overlapping regions become active during both performing and observing an action, not recordings of individual neurons (Rizzolatti & Craighero, 2004). A further popularized claim, that autism is caused by a "broken mirror neuron system," has not held up under more rigorous testing and is no longer considered well supported.

*Diagnostic question:* if someone tells you "mirror neurons prove humans understand each other's intentions by literally simulating them in our brains," what's missing from that claim? (The direct evidence — single neurons firing during both action and observation — exists only in monkeys; the human evidence is indirect; and the leap from "overlapping brain activity" to "understanding intentions," let alone to specific claims like the autism "broken mirror" hypothesis, outruns what either evidence base has actually shown.)
...
Every mechanism covered so far in this chapter describes *what* gets learned and under what conditions. This section turns to a different question: what is dopamine actually doing when reward and anticipation are involved? The most common popular description — that dopamine is simply the brain's pleasure signal — is, according to the evidence, backward.
...
This reframing changes how several things in this book should be understood, including content from earlier chapters. Chapter 5's account of stimulant drugs noted that cocaine and amphetamines flood the dopamine system — this section's point is that they are not simply manufacturing pleasure out of nothing; they are hijacking a system that, across the studies described above, tracks prediction and drives pursuit more reliably than it tracks pleasure, which is part of why stimulant addiction so often looks less like simple pleasure-seeking and more like compulsive, anxious pursuit that persists even once the drug stops reliably producing pleasure at all. The addiction literature has a specific name for this split: **wanting** (dopamine-driven incentive salience — the pull toward a cue or a reward) is not the same system as **liking** (the hedonic pleasure of actually getting it), and the two can drift apart, most dramatically in addiction, where wanting for a drug can stay intense — or even grow more intense with repeated use — long after the drug itself has stopped producing much liking at all (Berridge & Robinson, 1998). None of this makes dopamine one single, uniform signal, either — its effects depend on which pathway is active, which receptor types and cell populations are involved, what task is being performed, and over what timescale the response is measured, so a finding about prediction-error signaling in one circuit does not automatically generalize to everything dopamine does elsewhere in the brain. It also fits why variable-ratio reinforcement schedules (Section 2) are so powerfully resistant to extinction: a system that responds most strongly to uncertain, unpredictable outcomes is well-positioned to sustain exactly the kind of behavior a payoff schedule built entirely on uncertainty would produce.
...
| [Mirror neurons](#do-not-confuse-macaque-single-neuron-evidence-vs-human-mirror-system-evidence) | Ch. 10 — Lifespan Development (Theory of Mind) | A genuinely contested connection, flagged honestly here: mirror neurons are sometimes proposed as a biological substrate for understanding others' mental states, but — as this chapter notes — the strongest version of that claim (including the autism "broken mirror" hypothesis) has not held up well, which is itself a useful case study for Ch. 10's treatment of how an appealing mechanism can outrun its evidence |
| [Dopamine as prediction, not pleasure](#section-4-dopamine-reward-and-why-learning-feels-the-way-it-feels) | Ch. 5 — Consciousness (review) | This chapter delivers in full the dopamine mechanism Chapter 5 previewed when explaining how stimulant drugs hijack the brain's reward pathway |
...
**9.** A cat that once got its paw pinched in a cabinet door now freezes and backs away whenever it sees that cabinet, even when the door is wide open and nothing is inside. The same cat has also learned that pawing at the pantry door gets a human to open it and give a treat, so it paws at the pantry door more and more often. Which part of this scenario is classical conditioning, and which is operant conditioning?

a) Both parts are classical conditioning, since both are based on the cat's past experience  
b) The freezing at the cabinet is classical conditioning (a stimulus now elicits a fear response); the pawing at the pantry is operant conditioning (a behavior is shaped by its consequence)  
c) The freezing at the cabinet is operant conditioning; the pawing at the pantry is classical conditioning  
d) Both parts are operant conditioning, since the cat is actively doing something in both cases  

*Answer: b. Why (c) is tempting: it's easy to assume conditioning is "operant" whenever the cat is described as doing something specific, but freezing is elicited by a stimulus (the cabinet) that has become associated with a past aversive event — a stimulus-predicts-stimulus relationship, which is classical conditioning — while pawing is a behavior whose frequency is being shaped by what follows it (a treat), which is operant conditioning. What separates the two is whether a stimulus is predicting another stimulus or a behavior is predicting its own consequence, not whether the response looks active or passive.*

---

**10.** According to the dopamine research described in this chapter, dopamine-neuron activity in the reward pathway rises most sharply:
...
*Answer: b. Why (a) is tempting: dopamine is popularly described as a "pleasure chemical" that should peak when something good actually happens, but Schultz's and Fiorillo's findings show the opposite pattern — dopamine-neuron activity rises most sharply during anticipation of an uncertain reward, often more than at the moment the reward itself arrives.*
...
**Classical conditioning** — Learning in which a previously neutral stimulus (CS), through repeated pairing with a stimulus that already produces a response (UCS), comes to elicit a similar response (CR) on its own.
...
**Dopamine** — A neurotransmitter central to the brain's reward pathways; well-studied dopamine neurons signal reward prediction and reward uncertainty rather than pleasure itself, though the signal's effects vary by pathway, receptor, and context.
...
**Operant conditioning** — Learning in which a behavior's future frequency is shaped by the consequences (reinforcement or punishment) that follow it.
...
**Schedule of reinforcement** — The pattern by which reinforcement follows behavior. Fixed schedules (a set number of responses, or a set amount of time) produce predictable reinforcement that is quick to establish and quick to extinguish once reinforcement stops. Variable schedules (an unpredictable number of responses or amount of time) take longer to produce a stable response pattern but are far more resistant to extinction — the same unpredictability that slows acquisition is what makes the behavior hardest to unlearn.
