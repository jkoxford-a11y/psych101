...
> Draft v0.1 — 2026-06-22 | Status: new full first draft, not yet reviewed by Jon
> Word count target: 3,500–5,000 body words (Section 1 through end of Section 4). This chapter covers 27 MEM-* concepts — more than any chapter so far (Ch6's 17 was the previous high) — plus a full classic-study walkthrough (H.M.) with a contrast case (Clive Wearing), two Do Not Confuse panels, and two AI Connection passages. Flagging in advance that this will very likely land over target, consistent with how Ch3's and Ch6's overages were handled: flag, don't cut preemptively.
...
Chapter 6 covered learning — how experience changes future behavior. Memory is the system that makes that possible: without some way of encoding, storing, and retrieving past experience, nothing learned on Monday could affect what happens on Tuesday. This chapter also picks up a thread Chapter 5 deliberately left thin — sleep's role in memory consolidation was anchored there only briefly, with H.M.'s case mentioned and explicitly deferred to this chapter, where it now gets full treatment. Going forward, working memory's limited capacity (Section 1) becomes a recurring constraint in Chapter 8's account of problem-solving and reasoning, and this chapter's account of reconstructive memory sets up Chapter 10's treatment of how groups distort shared recollections.
...
**The simplification:** what gets left out is the central executive itself. Working memory's executive component does real cognitive work — it decides what is currently relevant, suppresses information that would interfere, and allocates limited attentional resources toward the parts of the held information that matter most for the task at hand. A context window has no equivalent. Every token inside it sits there with equal structural standing; nothing inside the system is actively triaging what's currently important the way your central executive triages a held phone number against a distracting conversation. The context window is sheer capacity without executive control layered on top of it.
...
## Section 4: When Memory Goes Wrong — Errors, Amnesia, and the Brain

This section delivers on the Misconception Opener's central claim: memory is reconstructive, and the clearest evidence for that claim comes from cases where reconstruction visibly goes wrong, producing confident, detailed, sincerely-held memories that are nonetheless false, distorted, or borrowed from the wrong source entirely.
...
**The simplification:** the resemblance stops at the surface. Human reconstruction is anchored, however loosely, to an actual encoded experience — even a badly distorted or source-confused memory traces back to *something* that was genuinely perceived, learned, or imagined by that person at some point, and is shaped by emotional salience, motivation, and (when working correctly) source-monitoring processes that at least attempt to track where information came from. An AI hallucination has no encoding event behind it at all. There is no original experience being reconstructed, badly or otherwise — the system is generating a statistically plausible continuation of text based on patterns in its training data, with nothing that corresponds to "a memory" sitting underneath the output waiting to be retrieved.
...
Long-term memory storage depends biologically on long-term potentiation, and functionally splits into explicit memory (episodic and semantic) and implicit memory (including procedural memory), which can be damaged independently of each other. Retrieval shows reliable patterns — the serial position effect's primacy and recency components — and forgetting is explained by both decay and interference (proactive and retroactive), with classic evidence favoring interference as the larger contributor to everyday forgetting. Memory is fundamentally reconstructive rather than reproductive: the misinformation effect, false memory, confabulation, and source misattribution are all different faces of the same underlying mechanism, vividly illustrated by flashbulb memories (vivid but not necessarily accurate) and definitively established by H.M.'s case, contrasted here with Clive Wearing's, both of which showed that explicit and procedural memory are genuinely separate systems supported by different brain structures.
...
|---|---|---|
| [Working memory (Baddeley model)](#section-1-encoding--building-the-trace) | Ch. 8 — Thinking, Language & Intelligence | Working memory capacity is a major individual-difference predictor of reasoning, problem-solving, and fluid intelligence measures |
| [H.M. and hippocampal memory](#classic-study-hm-and-the-case-for-multiple-memory-systems) | Ch. 3 — Neuroscience & Biological Bases (review) | This chapter supplies the behavioral evidence for what the hippocampus actually does — Ch. 3 introduced the structure, this chapter shows what its loss costs |
| [Memory consolidation](#classic-study-hm-and-the-case-for-multiple-memory-systems) | Ch. 5 — States of Consciousness (review) | Sleep-dependent memory consolidation was anchored briefly in Ch. 5 specifically to be completed here; this chapter delivers the fuller mechanistic story |
| [Misinformation effect / eyewitness testimony](#section-4-when-memory-goes-wrong--errors-amnesia-and-the-brain) | Ch. 10 — Social Psychology | Non-obvious: co-witness contamination of shared memories is a genuinely social phenomenon, not just an individual memory failure, and eyewitness identification errors have direct courtroom consequences this book returns to |
| [Spacing effect and testing effect](#section-1-encoding--building-the-trace) | Prologue — How to Actually Learn This Stuff | The prologue foreshadows these as evidence-based study strategies; this chapter supplies the encoding and retrieval mechanisms that explain why they work |
| [Flashbulb memories / emotional arousal and encoding](#section-4-when-memory-goes-wrong--errors-amnesia-and-the-brain) | Ch. 12 — Motivation & Emotion | Non-obvious: the same amygdala-mediated emotional arousal that makes flashbulb memories unusually vivid is covered mechanistically in full when emotion becomes the chapter's central topic |
| [Confabulation and source misattribution](#section-4-when-memory-goes-wrong--errors-amnesia-and-the-brain) | Ch. 14 — Psychological Disorders & Therapy | The recovered-memory controversy in clinical practice — therapists potentially helping construct false memories of abuse during treatment — is a direct, ethically loaded application of this chapter's reconstructive-memory content |
...
c) human confabulation traces back to some actual encoded experience, however distorted, while an AI hallucination has no encoding event behind it at all — it is a statistically plausible continuation of text, not a reconstruction of anything that happened
d) there is no meaningful similarity between the two phenomena at all

*Answer: c. Why (d) is tempting: it overcorrects — there is a real surface-level resemblance this chapter draws deliberately (both involve confident, fluent, false output with no intent to deceive) — but the breakdown is specifically that human reconstruction is anchored to a real, if imperfectly encoded, experience, while an AI hallucination has no analogous encoding event of any kind behind it.*
...
**Long-term potentiation (LTP)** — A lasting strengthening of the connection between two neurons that fire together repeatedly, the cellular basis of durable memory storage.
...
Elizabeth Loftus's own module on misinformation, false memory, and eyewitness identification — the primary source this chapter drew on for the confabulation and source-misattribution content.
...
A deeper dive into the recovered-memory controversy mentioned in this chapter's Connections table, foreshadowing Chapter 14.
