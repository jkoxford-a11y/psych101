# Chapter 5: Consciousness — Processing, Experience, and Access

> Drafting history & provenance: see `_provenance/ch05-consciousness.md` (and git log).
...
![A cortically blind man walks down an obstacle-filled hallway, successfully weaving around cones and a plant while reporting no visual awareness of them. His navigation shows visually guided processing even though he reports no ordinary visual experience.](../images/ch05/blind_sight.png)
*Blindsight separates visually guided action from reported seeing. Information about the obstacles shaped navigation without becoming ordinary visual experience (de Gelder et al., 2008).*
...
Alt text: A three-part dashboard asks separate questions about conscious state, conscious content, and conscious access. Short case markers show that blindsight, Charles Bonnet syndrome, alcohol blackout, psychedelic states, and anesthesia alter different parts of the dashboard rather than moving one overall consciousness dial.
Caption: Consciousness is better approached as a multidimensional dashboard than as one dimmer switch. State asks about the organism's overall condition, content asks what is experienced, and access asks what information can guide report and flexible use. The cases in this chapter change these dimensions in different combinations.
Status: proposed flagship figure; artwork not yet created
-->
...
> **Stop and Retrieve:** A blindsight patient turns around an obstacle while reporting that nothing is visible. Describe the case using conscious state, conscious content, and conscious access. Then name the processing that occurred outside ordinary visual experience.
...
> **Do Not Confuse: Experience vs. the Report of Experience**
>
...
3. mismatch produces a **prediction error**;
4. predictions, attention, or action change.
...
> **Do Not Confuse: Blacking Out vs. Passing Out**
>
...
a) conscious state is absent, but visual content is normal
b) wakeful state and visually guided processing are present, but ordinary visual content and reportable access are absent
c) visual content is present because all successful action requires seeing
d) access is complete because the patient can move
...
a) the dimensions are a teaching synthesis informed by multidimensional research, not a universally settled taxonomy
b) researchers have proved that exactly three brain regions create consciousness
c) the dimensions never interact
d) the dashboard is only a metaphor for drug dosage
...
a) why any physical process feels like something
b) how selected information may become amplified and broadly available for memory, evaluation, language, and flexible action
c) why every semantic process requires awareness
d) how serotonin receptors generate all conscious content
...
a) every unresponsive patient is conscious
b) bedside responsiveness can underestimate preserved cognition in some cases
c) fMRI directly measures phenomenal experience
d) motor behavior is scientifically irrelevant
...
a) episodic encoding and momentary experience are separable achievements
b) all blackouts are actually episodes of passing out
c) alcohol leaves memory systems unchanged
d) behavior stops completely during a blackout
...
a) the anesthetized patients consciously understood every word
b) residual neural representation can persist despite loss of responsiveness, but representation is not proof of conscious comprehension
c) anesthesia affects every drug, dose, and brain network identically
d) responsiveness and neural processing are the same measure
...
**Alcohol blackout** — Alcohol-related amnesia for some or all events during a period in which a person may remain awake and behaviorally active.

**Automaticity** — The capacity for a practiced behavior to run with reduced conscious monitoring while remaining responsive to relevant information.

**Blindsight** — Visually guided responding in a cortically blind field without reported ordinary visual experience.

**Charles Bonnet syndrome** — Visual hallucinations associated with significant vision loss, commonly accompanied by insight that the images are unreal.

**Cocktail-party effect** — The capture of attention by personally important information, such as one’s name, in an unattended auditory stream.

**Cognitive motor dissociation** — Evidence of command-following cognitive activity on fMRI or EEG in a patient who shows no observable motor response to commands.

**Conscious access** — Availability of information for report, reasoning, memory, evaluation, and flexible action.
...
**General anesthesia** — A drug-induced condition that alters conscious state, responsiveness, memory, sensory processing, and network coordination.

**Global-workspace theory** — A candidate account proposing that selected information becomes amplified and broadly available to specialized systems.

**Hard problem of consciousness** — Chalmers’s term for the question of why physical or computational processing is accompanied by subjective experience.

**Inattentional blindness** — Failure to report a visible object because attention is strongly engaged elsewhere.

**No-report paradigm** — A method that infers conscious content without requiring an explicit report on every trial, often using physiological, behavioral, or neural indicators.
...
**Predictive processing** — A framework in which perceptual systems combine prior information with sensory evidence to construct and revise hypotheses.
